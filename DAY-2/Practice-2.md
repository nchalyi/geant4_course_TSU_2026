...
Далее `.hh` файлы мы будем просто объявлять без комментрирования.
...
5. Теперь запустим сначала в UI режиме
```bash
./myproject
```

6. В открывшемся окне запустим команду `/run/initialize`, а затем откроем `vis.mac`. После этого можно запустить пучок наших частиц через `/run/beamOn`.
